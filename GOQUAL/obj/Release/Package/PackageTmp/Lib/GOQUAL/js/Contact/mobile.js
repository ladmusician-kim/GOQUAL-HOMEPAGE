       ...
       controller('ContactCtrl', function ($scope, $timeout, ngTableParams) {
           ...
           $scope.sendMessage = function () {
               getJson('/Contact/SendMessage', { name: $scope.name, email: $scope.email, phone: $scope.phone, content: $scope.content },
                   function (data) {
                       alert($scope.name + '님 소중한 의견 감사합니다. 답변이 필요한 문의일 경우 빠른 시일내로 연락을 드리겠습니다.');
                   },
                   function (arg) {
                       alert($scope.name + '님께서 문의하신 내용이 프로그램 문제로 인해 발송되지 않았습니다. gq@goqual.com으로 문의해주세요');
                   }, $scope);
           }
           ...
           $scope.detailAdvise = function (item) {
               if (item.password == "") {
                   alert('비밀번호를 입력해주세요', 'makalu-error', 'error');
               } else {
                   getJson('/Contact/ConfirmPassword', { id: item.Id, password: item.password },
                   function (data) {
                       if (data > 0) {
                           window.location.href = "/Contact/Mo_Detail?id=" + item.Id;
                       } else {
